...
export function getAllMembers({ allPages }) {
  ...
  return sortMembers(slim)
}

/**
 * 获取首页散点展示用的极简成员数据
 * 只保留 CommunityRolesSection 需要的 7 个字段
 */
export function getMembersForScatter({ allMembers }) {
...
/**
 * 获取成员目录列表用的轻量成员数据。
 * 目录页不需要社交链接、页面封面等详情字段，避免点击导航时解析过大的 pageProps。
 */
export function getMembersForDirectory({ allMembers }) {
  if (!Array.isArray(allMembers)) return []
  return allMembers.map(m => ({
    id: m.id || '',
    title: m.title || '',
...
export function getAllRecords({ allPages }) {
  if (!Array.isArray(allPages)) {
    return []
  }

  return [...allPages]
    .filter(page => page?.type === 'Record' && page?.status === 'Published')
    .sort((a, b) => {
      const aDate = a?.ext?.timelineDate
      const bDate = b?.ext?.timelineDate
...
function isTruthyMemberFlag(value) {
  if (Array.isArray(value)) {
    return value.some(item => isTruthyMemberFlag(item))
  }
  if (typeof value === 'boolean') return value
  if (typeof value === 'number') return value > 0
  if (typeof value === 'string') {
    return ['true', '1', 'yes', 'y', 'on', 'featured'].includes(
...
async function convertNotionToSiteData(
  ...
  // 补充：通过官方 API 读取 Member 条目（非官方 API 的数据库视图可能不包含 Member 类型）
  ...
  if (officialMembers.length > 0) {
    ...
    const newMembers = officialMembers.filter(m => !existingIds.has(m.id))
    if (newMembers.length > 0) {
      allPages = [...allPages, ...newMembers]
    }
  }

  // 补充：通过官方 API 读取 Event 条目，避免 collection view 漏掉活动行
  const officialEvents = await fetchEventsFromOfficialAPI()
  if (officialEvents.length > 0) {
    const existingIds = new Set(allPages.map(p => p.id))
    const newEvents = officialEvents.filter(event => !existingIds.has(event.id))
    if (newEvents.length > 0) {
      allPages = [...allPages, ...newEvents]
    }
  }

  // 补充：通过官方 API 读取 Record 条目（社区现场），collection view 不含 Record 类型
  const officialRecords = await fetchRecordsFromOfficialAPI()
  if (officialRecords.length > 0) {
    const existingIds = new Set(allPages.map(p => p.id))
    const newRecords = officialRecords.filter(record => !existingIds.has(record.id))
    if (newRecords.length > 0) {
      allPages = [...allPages, ...newRecords]
    }
  }
  ...
  const allRecords = getAllRecords({ allPages })
...
function handleDataBeforeReturn(db) {
  ...
  db.allRecords = cleanPages(db.allRecords, db.tagOptions) || []
...
function cleanPages(allPages, tagOptions) {
  ...
  allPages.forEach(page => {
    ...
    if (page?.password) {
      Object.assign(page, redactPasswordProtectedSummary(page))
    }
  })
  return allPages
}
...
/**
 * 通过官方 API 补充读取 Member 条目
 * NotionNext 非官方 API 的数据库视图可能不包含 Member 类型
 * 使用 notion-client 的 getDataSource 方法查询 data_source
 */
export async function fetchMembersFromOfficialAPI() {
  ...
  try {
    ...
    const memberType =
      process.env.NOTION_MEMBERS_TYPE_VALUE ||
      BLOG.NOTION_PROPERTY_NAME.type_member ||
      'Member'
    const publishedStatus =
      process.env.NOTION_MEMBERS_STATUS_PUBLISHED_VALUE ||
      BLOG.NOTION_PROPERTY_NAME.status_publish ||
      'Published'

    // Step 1: 获取 data_source schema
    const dsResponse = await notionFetch(`/data_sources/${dataSourceId}`, { headers })
    if (!dsResponse.ok) return []
    const dataSource = await dsResponse.json()

    // 找到各属性的内部 key
    const typeKey = findDataSourcePropertyKey(dataSource, propertyCandidates.type)
    const statusKey = findDataSourcePropertyKey(
      dataSource,
      propertyCandidates.status
    )
    if (!typeKey || !statusKey) return []

    // Step 2: 查询 data_source（分页，获取所有 Member）
    let allResults = []
    let cursor = null
    do {
      const body = { page_size: 100 }
      if (cursor) body.start_cursor = cursor
      const queryResponse = await notionFetch(`/data_sources/${dataSourceId}/query`, {
        method: 'POST',
        headers,
    ...
    // Step 3: 过滤 Type=Member 且 Status=Published
    const members = allResults.filter(page => {
      const type = readDataSourcePropertyValue(page.properties?.[typeKey])
      const status = readDataSourcePropertyValue(page.properties?.[statusKey])
      return type === memberType && status === publishedStatus
    })

    // Step 4: 映射属性
    return removeUndefinedValues(members.map(page => mapOfficialMemberPage(page, dataSource)))
  } catch (err) {
    console.warn('[MEMBER-SYNC] Failed to fetch Members:', err.message)
    return []
  }
}

/**
 * 通过官方 API 补充读取 Event 条目
 * 活动与成员共用社区 data_source，但 NotionNext 非官方 collection view
 * 可能只返回当前视图里的行，因此 Event 也需要独立补拉。
 */
export async function fetchEventsFromOfficialAPI() {
  ...
  try {
    ...
    const eventType =
      process.env.NOTION_EVENTS_TYPE_VALUE ||
      BLOG.NOTION_PROPERTY_NAME.type_event ||
      'Event'
    const publishedStatus =
      process.env.NOTION_EVENTS_STATUS_PUBLISHED_VALUE ||
      BLOG.NOTION_PROPERTY_NAME.status_publish ||
      'Published'
    ...
    const events = allResults.filter(page => {
      const type = readDataSourcePropertyValue(page.properties?.[typeKey])
      const status = readDataSourcePropertyValue(page.properties?.[statusKey])
      return type === eventType && status === publishedStatus
    })

    // Fetch full page details (including cover) for each event
    const eventsWithCover = await Promise.all(
      events.map(async page => {
        try {
          const pageResponse = await notionFetch(`/pages/${page.id}`, { headers })
          if (pageResponse.ok) {
            const fullPage = await pageResponse.json()
            // Merge cover from full page response
            return { ...page, cover: fullPage.cover }
          }
        } catch (err) {
          console.warn(`[EVENT-SYNC] Failed to fetch page ${page.id}:`, err.message)
        }
        return page
      })
    )

    return removeUndefinedValues(eventsWithCover.map(page => mapOfficialEventPage(page, dataSource)))
  } catch (err) {
    console.warn('[EVENT-SYNC] Failed to fetch Events:', err.message)
    return []
  }
}

/**
 * 通过官方 API 补充读取 Record 条目（社区现场）
 * Record 类型在非官方 collection view 里读不到，必须走 data_source API。
 */
export async function fetchRecordsFromOfficialAPI() {
  const token = process.env.NOTION_API_TOKEN
  const dataSourceId =
  ...
  try {
    ...
    const recordType =
      process.env.NOTION_RECORDS_TYPE_VALUE ||
      BLOG.NOTION_PROPERTY_NAME?.type_record ||
      'Record'
    const publishedStatus =
      process.env.NOTION_RECORDS_STATUS_PUBLISHED_VALUE ||
      BLOG.NOTION_PROPERTY_NAME?.status_publish ||
      'Published'
    ...
    const records = allResults.filter(page => {
      const type = readDataSourcePropertyValue(page.properties?.[typeKey])
      const status = readDataSourcePropertyValue(page.properties?.[statusKey])
      return type === recordType && status === publishedStatus
    })

    // Fetch full page details (including cover) for each record
    const recordsWithCover = await Promise.all(
      records.map(async page => {
        try {
          ...
          console.warn(`[RECORD-SYNC] Failed to fetch page ${page.id}:`, err.message)
        }
        return page
      })
    )

    return removeUndefinedValues(recordsWithCover.map(page => mapOfficialRecordPage(page, dataSource)))
  } catch (err) {
    console.warn('[RECORD-SYNC] Failed to fetch Records:', err.message)
    return []
  }
}
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle = BLOG.TITLE || NOTION_CONFIG?.TITLE || 'IGNAI'
  const defaultDescription =
    BLOG.DESCRIPTION || NOTION_CONFIG?.DESCRIPTION || '长沙 AI 社区 — 连接创造者，点燃可能性'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || BLOG.HOME_BANNER_IMAGE || '/bg_image.jpg'
  const defaultIcon = NOTION_CONFIG?.AVATAR || BLOG.AVATAR || '/avatar.svg'
  const defaultLink = BLOG.LINK || NOTION_CONFIG?.LINK
  ...
  const notionTitle = collection?.name?.[0]?.[0] || ''
  const notionDescription = collection?.description
    ? Object.assign(collection).description[0][0]
    : ''
  const title = defaultTitle || notionTitle
  const description = defaultDescription || notionDescription
  // IGNAI 的站点壳由 blog.config.js 控制，避免旧 Notion 模板封面进入公开 SEO。
  const pageCover = defaultPageCover || (collection?.cover
  ...
  const link = defaultLink || NOTION_CONFIG?.LINK
