...
export function normalizeNotionEvent(event) {
  ...
  // Read top-level category (Chinese labels), map to English kind.
  ...
  const kindFromCategory = categoryToKind[event?.category]

  // Read from top-level date property (supports start/end for ranges).
  const dateStart = event?.date?.start
  const dateEnd = event?.date?.end
  const dateFromProperty = dateStart
    ? dateEnd
      ? `${dateStart} – ${dateEnd}`
      : dateStart
    : null
  ...
  return {
    ...
    subtitle: readFirstString(event?.summary),
    ...
    location: readFirstString(event?.location, '待定'),
    format: normalizeEventFormat(event?.format),
    ...
    coverPosition: readFirstString(event?.coverPosition, 'center'),
    externalUrl,
    excerpt: readFirstString(event?.summary, ''),
    tags: readFirstArray(event?.tags),
    ...
    audience: readFirstArray(event?.audience),
    agenda: readFirstArray(event?.agenda),
    hosts: readFirstArray(event?.hosts, event?.organizer_slugs),
    notes: readFirstArray(event?.notes),
    relatedRecordSlugs: []
  }
}
